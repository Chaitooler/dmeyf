...
hiper$programa       <-  "rpart_tune_MBO_meses.r"
...
modelo_rpart_ganancia_MBO_directo = function(x = list(pmaxdepth, pminbucket, pminsplit, pcp, pventana ) )
{
  ...
  mactual  <-  modelo_rpart( dataset_grande[ mes<=(env$data$mes_actual_train+x$pventana-1)  &  mes>=env$data$mes_actual_train,],  dataset_grande[ mes==env$data$mes_actual_test, ], env$data$clase_nomcampo, env$data$clase_valor_positivo, env$problema, pmaxdepth=x$pmaxdepth,  pminbucket=vminbucket, pminsplit=x$pminsplit, pcp=x$pcp )
  mfuturo  <-  modelo_rpart( dataset_grande[ mes<=(env$data$mes_futuro_train+x$pventana-1)  &  mes>=env$data$mes_futuro_train,],  dataset_grande[ mes==env$data$mes_futuro_test, ], env$data$clase_nomcampo, env$data$clase_valor_positivo, env$problema, pmaxdepth=x$pmaxdepth,  pminbucket=vminbucket, pminsplit=x$pminsplit, pcp=x$pcp )
