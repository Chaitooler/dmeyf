...
data$archivo_actual_train <-  "201812_dias.txt"
...
hiper$programa       <-  "rpart_tune_MBO_02.r"
hiper$algoritmo      <-  "rpart"
hiper$busqueda       <-  "MBO"
hiper$estimacion     <-  "mes_futuro"
hiper$observaciones  <-  "basics"
hiper$separador      <-  "\t"
...
#------------------------------------------------------------------------------
x <- list( pmaxdepth=10, pminbucket=10, pminsplit=10, pcp=0 )

modelo_rpart_ganancia_MBO_directo = function(x = list(pmaxdepth, pminbucket, pminsplit, pcp ) )
{
  ...
  mactual  <-  modelo_rpart( dataset_actual_train,  dataset_actual_test, env$data$clase_nomcampo, env$data$clase_valor_positivo, env$problema, pmaxdepth=x$pmaxdepth,  pminbucket=vminbucket, pminsplit=x$pminsplit, pcp=x$pcp )
...
dataset_actual_train  <- fread(cmd=paste("cat", env$data$archivo_actual_train))
dataset_actual_test   <- fread(cmd=paste("cat", env$data$archivo_actual_test))

dataset_futuro_train  <- fread(cmd=paste("cat", env$data$archivo_futuro_train))
dataset_futuro_test   <- fread(cmd=paste("cat", env$data$archivo_futuro_test))

#borro las variables que no me interesan
dataset_actual_train[,  (env$data$campos_a_borrar) := NULL    ] 
dataset_actual_test[ ,  (env$data$campos_a_borrar) := NULL    ] 
dataset_futuro_train[ ,  (env$data$campos_a_borrar) := NULL    ] 
dataset_futuro_test[ ,  (env$data$campos_a_borrar) := NULL    ] 

dataset_futuro_train  <-  dataset_futuro_test


#uso esta semilla para los canaritos
set.seed(10219)
canaritos_idx <- 0.2 
canaritos_cantidad <- as.integer( round(ncol(dataset_actual_train)*canaritos_idx) )
for( i in 1:canaritos_cantidad )
{
  dataset_actual_train[  , paste0( "canarito", i ) :=  runif( nrow(dataset_actual_train) ) ]
  dataset_actual_test[   , paste0( "canarito", i ) :=  runif( nrow(dataset_actual_test) ) ]
