...
#atencion  rpart toma malas decisiones si se deja minbucket=1 por lo que se lo establecio en 6
#en  rpart_canarito_weight.r  si se trabaja con  minbucket=1
...
#source("M:\\R\\rpart\\rpart_canarito.r")
...
karchivo_imagen        <-  "rpart10_binaria_canarito.jpg"
karchivo_imagen_pruned <-  "rpart10_binaria_canarito_pruned.jpg"

#------------------------------------------------------------------------------

pdataset <- dataset_original 
pclase   <- "clase_ternaria"
porigen  <- "origen"

romper  <- function(  pdataset, pclase, porigen )
{
  vcampos <- setdiff( colnames( pdataset ), c( pclase, porigen ) )
  vcant_registros <-  nrow( pdataset )
   
  for( campo in  vcampos )
  {
    pdataset[  , azar :=  runif( vcant_registros ) ]
    setorderv(  pdataset,  c( campo, "azar" ) )
    pdataset[   ,  eval(campo) :=  ifelse( is.na(get(campo)), NA, 1:vcant_registros ) ]
  }
...
#dataset_actual[  , origen:= 1 ]
#dataset_futuro[  , origen:= 2 ]

#dataset_unido <-  rbind( dataset_actual, dataset_futuro )

#dataset_original <-  copy( dataset_unido )

#romper( dataset_unido, "clase_ternaria", "origen" )


#dataset_actual  <-  dataset_unido[  origen==1 , ]
#dataset_futuro  <-  dataset_unido[  origen==2 , ]
...
modelo_original   <-  rpart(formula,   data = dataset_actual,   xval=0, cp=0.0, maxdepth=30, minbucket=vminbucket, minsplit= vminbucket)
