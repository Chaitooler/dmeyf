...
modelo_rpart_particion = function( pdataset, psemilla, pclase_nomcampo, pclase_valor_positivo, pproblema, pmaxdepth, pminbucket, pminsplit, pcp )
{

  formula  <-  formula(paste(pclase_nomcampo, " ~ ",
                             paste0(setdiff(colnames(pdataset), c(pclase_nomcampo,"particion")), collapse=" + " ) )
                      )

  modelo   <-  rpart(formula,   data = pdataset[ substr(particion, psemilla, psemilla)=="1", ], 
                      xval=0, maxdepth=pmaxdepth, minbucket=pminbucket, minsplit=pminsplit, cp=pcp)


  #aplico el modelo a datos nuevos
  testing_prediccion  <- predict( modelo, 
                                  pdataset[ substr(particion, psemilla, psemilla)=="0", ], 
                                  type = "prob")


  # calculo la ganancia normalizada  en testing
  gan  <-  fmetrica_ganancia_rpart(testing_prediccion[, pclase_valor_positivo ],  pdataset[ substr(particion, psemilla, psemilla)=="0", get(pclase_nomcampo)], pclase_valor_positivo, pproblema)
  # calculo el AUC en testing
  auc  <-  fmetrica_auc_rpart(testing_prediccion[, pclase_valor_positivo ],  pdataset[ substr(particion, psemilla, psemilla)=="0", get(pclase_nomcampo)], pclase_valor_positivo )
...
modelo_rpart_ganancia_MBO_semilla = function( psemilla, pdataset, pclase_nomcampo, pclase_valor_positivo, pproblema, pmaxdepth, pminbucket, pminsplit, pcp  )
{
  ...
  res <-  modelo_rpart_particion( pdataset, psemilla, pclase_nomcampo, pclase_valor_positivo, pproblema, pmaxdepth, pminbucket, pminsplit, pcp  )
...
modelo_rpart_ganancia_MBO_montecarlo = function(x = list(pmaxdepth, pminbucket, pminsplit, pcp ) )
{
  ...
  vv <- 1:length( env$montecarlo$semilla_azar )
 
  t0   <-  Sys.time()

  mactual  <- mclapply( vv,  modelo_rpart_ganancia_MBO_semilla, 
...
particionar_dataset <-  function( pdataset, psemillas )
{
  vlargo <- length(psemillas)
  
  st <- paste(replicate(vlargo, "0"), collapse = "")
  pdataset[ , particion:= st ]

  for( i in 1:vlargo )
  {
    #Divido el dataset en training 70% y testing 30%
    set.seed( psemillas[i] )
    ...
    tb_clase[  , training := as.integer((.I %% 10)<7 )]
  
    setorder( tb_clase, idtempo )
    pdataset[  , particion:=   paste0( substr(particion, 1, i-1), tb_clase$training, substr(particion,i+1,vlargo)) ]
...
particionar_dataset(  dataset_actual, env$montecarlo$semilla_azar )
...
columnas_canaritos <-   colnames( dataset_actual )[ which( colnames( dataset_actual ) %like% "canarito" ) ]

setcolorder(  dataset_actual,  unique( c( columnas_canaritos, colnames(dataset_actual) ) ) )
setcolorder(  dataset_futuro_train,  unique( c( columnas_canaritos, colnames(dataset_futuro_train) ) ) )
setcolorder(  dataset_futuro_test,  unique( c( columnas_canaritos, colnames(dataset_futuro_test) ) ) )
...
obj.fun <- makeSingleObjectiveFunction(
        ...
        par.set = makeParamSet(
            ...
            makeNumericParam("pcp"               ,  lower=-0.01 , upper= 0.01)
        ),
