...
hiper$programa       <-  "rpart_tune_MBO_meses_undersampling.r"
...
modelo_rpart <-  function( ptrain, ptest, pclase_nomcampo, pclase_valor_positivo, pproblema, pmaxdepth, pminbucket, pminsplit, pcp )
{
  ...
  #jpeg(file = "moco.jpg",  width = 100, height = 12, units = 'in', res = 300)
  ...
  gan  <-  fmetrica_ganancia(testing_prediccion[, pclase_valor_positivo ],  ptest[, get(pclase_nomcampo)], pclase_valor_positivo, problema)
  # calculo el AUC en testing
  auc  <-  fmetrica_auc(testing_prediccion[, pclase_valor_positivo ],  ptest[, get(pclase_nomcampo)], pclase_valor_positivo )
  ...
  return( list("ganancia_test"=gan, "auc_test"=auc, "canaritos_muertos"=  canaritos_muertos, "importancia"=modelo$variable.importance) )
}
#------------------------------------------------------------------------------
...
modelo_rpart_ganancia_MBO_directo <-  function(x = list(pmaxdepth, pminbucket, pminsplit, pcp, pventana ) )
{
  ...
  mactual  <-  modelo_rpart( dataset_grande[ (sample < env$undersampling | clase_ternaria=="BAJA+2") & mes<=(env$data$mes_actual_train+x$pventana-1)  &  mes>=env$data$mes_actual_train & mes!=env$data$mes_actual_test,],  
  ...
  mfuturo  <-  modelo_rpart( dataset_grande[ (sample < env$undersampling | clase_ternaria=="BAJA+2") & mes<=(env$data$mes_futuro_train+x$pventana-1)  &  mes>=env$data$mes_futuro_train & mes!=env$data$mes_futuro_test,],  
...
#Genero el modelo  
mfinal  <-  rpart(formula,
                  data = train_final,
                  xval=0, 
                  maxdepth= run$x$pmaxdepth, 
                  minbucket= run$x$pminbucket, 
...
tb_prediccion <-  as.data.table( cbind(   test_final[ , get(env$data$campo_id) ] , testing_prediccion[, env$data$clase_valor_positivo ] ) )
