...
dataset   <-   fread( "paquete_premium_exthist_modelitosxval.txt.gz" )
...
set.seed( 441461 ) #mi querida random seed, para que las corridas sean reproducibles
...
## Otros meses
daplicacion1  <-   xgb.DMatrix( data  = data.matrix( dataset[ foto_mes==201903, !c("numero_de_cliente","clase_ternaria"), with=FALSE]),
                                            label = dataset[ foto_mes==201903, clase_ternaria ]
)
aplicacion_prediccion1  <- predict(  modelo, daplicacion1 )
prediccion_final1  <-  cbind(  dataset[ foto_mes==201903, c("numero_de_cliente","clase_ternaria") ], aplicacion_prediccion1 )
colnames( prediccion_final1 )  <-  c( "numero_de_cliente", "clase01", "prob_positivo" )
sum(  prediccion_final1[ prob_positivo>0.025,   ifelse( clase01 == 1  , 19500, -500) ] )
...
fwrite( prediccion_final[ order( -prob_positivo) ], 
        file="lineademuerte_chris_probabilidades_UBA.txt", 
        sep="\t", 
        eol = "\r\n")
...
fwrite( as.data.table( prediccion_final[ prob_positivo > 0.025  , "numero_de_cliente" ] ), 
        file="lineademuerte_chris_entregar_UBA.txt", 
        col.names=FALSE, 
        sep="\t", 
        eol = "\r\n")
...
write.table(  xgb.importance( model = modelo )
              , file= "lineademuerte_chris_importancia_UBA.txt"
              , sep="\t"
              , eol = "\r\n"
           )
